...
site_info = pd.read_csv('04_analysis/out/basin_char_w_clusters_6_hydro_terranes.csv',  dtype = {'site_no':str})
...
for group in np.sort(site_info.hydro_terrane.unique()):
    ...
    run_id = os.path.join(model_run_id,  'Terrane_0'+str(group))
    out_dir = os.path.join(model_run_dir,  'Terrane_0'+str(group))
    
    read_input_data_from_file = False
    input_file_loc = os.path.join(model_run_dir,'Terrane_0'+str(group))
